...
describe('validation plugin - semantic - content-types - swagger2', function() {
  ...
  it('should complain when text or xml is used', function() {
    ...
    expect(res.errors[0].message).toEqual(`Global produces Content-Type must be JSON ('application/json' or 'application/hal+json').`);
    expect(res.errors[1].path).toEqual('consumes.0');
    expect(res.errors[1].message).toEqual(`Global consumes Content-Type must be JSON ('application/json' or 'application/hal+json').`);
    expect(res.warnings.length).toEqual(0);
  });
